...
#include <omnidirection_car/FrameToFrame.h>
#include <omnidirection_car/KinematicsInverse.h>
#include <omnidirection_car/Movement.h>
#include <omnidirection_car/MovementBezier.h>
#include <omnidirection_car/MovementGeneric.h>
#include <omnidirection_car/Velocity.h>
...
omnidirection_car::FrameToFrame mobileToWorld;

long double I;

omnidirection_car::KinematicsInverse kinematicsInverse;
...
omnidirection_car::Velocity velocity;
...
void onCommandMessage(const omnidirection_car::Movement::ConstPtr& input){
    if (input->movement == omnidirection_car::Movement::BEZIER) {
        if (input->bezier.frame == omnidirection_car::Movement::MOBILE) {
    ...
    } else if (input->movement == omnidirection_car::Movement::WHEEL) {
    ...
    } else if (input->movement == omnidirection_car::Movement::GENERIC) {
        if (input->generic.type == omnidirection_car::Movement::POSITION_ABSOLUTE) {
            if (input->generic.frame == omnidirection_car::Movement::WORLD) {
            ...
            } else if (input->generic.frame == omnidirection_car::Movement::MOBILE) {
            ...
            } else {//input->generic.frame == omnidirection_car::Movement::MOBILE_RAW
        ...
        } else if (input->generic.type == omnidirection_car::Movement::POSITION_RELATIVE) {
            if (input->generic.frame == omnidirection_car::Movement::WORLD) {
        ...
        } else {//input->generic.type == omnidirection_car::Movement::VELOCITY
            if (input->generic.frame == omnidirection_car::Movement::MOBILE) {
            ...
            } else if (input->generic.frame == omnidirection_car::Movement::WORLD) {
            ...
            } else {//input->generic.frame == omnidirection_car::Movement::HYBRID
...
int main(int argc, char **argv) {
    ...
    kinematicsInverseMobileClient = node.serviceClient<omnidirection_car::KinematicsInverse>("kinematics_inverse_mobile");
    kinematicsInverseWorldClient  = node.serviceClient<omnidirection_car::KinematicsInverse>("kinematics_inverse_world" );
    kinematicsMobileToWorldClient = node.serviceClient<omnidirection_car::FrameToFrame>("kinematics_mobile_to_world");
