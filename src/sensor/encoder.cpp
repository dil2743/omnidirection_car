...
omnidirection_car::Velocity message;
...
int main(int argc, char **argv){
    ...
    ros::Subscriber subscriber = node.subscribe("/omnidirection_car/joint_states", 1, onJointStateMessage);
    publisher = node.advertise<omnidirection_car::Velocity>("wheel_velocity", 1);
    ros::spin();
    return 0;
}
